...
export default function Home() {
  ...
  function playFile(file:string,key:string,onSuccess?:()=>void){
    ...
    const clip = new Audio(basePath+'/audio/'+file+'.wav'); audio.current=clip; setPlaying(key);
    ...
    <main className="main">
      ...
      <section className="game-board" aria-label={`Belajar ${fruit.idn}`}>
        <div className="picture-side" style={{'--fruit-tint':fruit.tint} as React.CSSProperties}>
          ...
          <button className={`fruit-button ${playing==='fruit'?'is-playing':''}`} onClick={()=>listen(round,'fruit')} aria-label={`Dengarkan nama ${fruit.idn} dalam bahasa ${language==='en'?'Inggris':'Arab'}`}><img src={basePath+`/fruits/${fruit.id}.webp`} alt={fruit.idn} draggable={false}/><span className="fruit-sound"><Volume2 size={25}/></span></button>
